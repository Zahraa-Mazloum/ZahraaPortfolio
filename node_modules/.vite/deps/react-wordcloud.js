...
var require_d3_cloud = __commonJS({
  "node_modules/d3-cloud/index.js"(exports, module) {
    ...
    module.exports = function() {
      var size = [256, 256], text = cloudText, font = cloudFont, fontSize = cloudFontSize, fontStyle = cloudFontNormal, fontWeight = cloudFontNormal, rotate = cloudRotate, padding = cloudPadding, spiral = archimedeanSpiral, words = [], timeInterval = Infinity, event2 = dispatch2("word", "end"), timer2 = null, random2 = Math.random, cloud = {}, canvas = cloudCanvas;
...
function nap() {
  var t04, t14 = taskHead, t23, time2 = Infinity;
  while (t14) {
    if (t14._call) {
      if (time2 > t14._time)
        time2 = t14._time;
...
function rgbSpline(spline) {
  return function(colors) {
    var n2 = colors.length, r2 = new Array(n2), g = new Array(n2), b = new Array(n2), i, color3;
    for (i = 0; i < n2; ++i) {
      color3 = rgb(colors[i]);
      r2[i] = color3.r || 0;
      g[i] = color3.g || 0;
...
function hsl2(hue3) {
  return function(start3, end2) {
    var h2 = hue3((start3 = hsl(start3)).h, (end2 = hsl(end2)).h), s = nogamma(start3.s, end2.s), l = nogamma(start3.l, end2.l), opacity = nogamma(start3.opacity, end2.opacity);
    return function(t4) {
      start3.h = h2(t4);
      start3.s = s(t4);
      start3.l = l(t4);
...
function hcl2(hue3) {
  return function(start3, end2) {
    var h2 = hue3((start3 = hcl(start3)).h, (end2 = hcl(end2)).h), c3 = nogamma(start3.c, end2.c), l = nogamma(start3.l, end2.l), opacity = nogamma(start3.opacity, end2.opacity);
    return function(t4) {
      start3.h = h2(t4);
      start3.c = c3(t4);
      start3.l = l(t4);
...
function cubehelix2(hue3) {
  return function cubehelixGamma(y) {
    y = +y;
    function cubehelix5(start3, end2) {
      var h2 = hue3((start3 = cubehelix(start3)).h, (end2 = cubehelix(end2)).h), s = nogamma(start3.s, end2.s), l = nogamma(start3.l, end2.l), opacity = nogamma(start3.opacity, end2.opacity);
      return function(t4) {
        start3.h = h2(t4);
        start3.s = s(t4);
...
function ordinal() {
  ...
  function scale(d2) {
    var key = d2 + "", i = index2.get(key);
    if (!i) {
      if (unknown !== implicit)
        return unknown;
      index2.set(key, i = domain.push(d2));
    }
...
var rgb_default2 = function rgbGamma2(y) {
  var color3 = gamma2(y);
  function rgb3(start3, end2) {
    var r2 = color3((start3 = rgb2(start3)).r, (end2 = rgb2(end2)).r), g = color3(start3.g, end2.g), b = color3(start3.b, end2.b), opacity = nogamma2(start3.opacity, end2.opacity);
    return function(t4) {
      start3.r = r2(t4);
      start3.g = g(t4);
      start3.b = b(t4);
...
function hsl4(hue3) {
  return function(start3, end2) {
    var h2 = hue3((start3 = hsl3(start3)).h, (end2 = hsl3(end2)).h), s = nogamma2(start3.s, end2.s), l = nogamma2(start3.l, end2.l), opacity = nogamma2(start3.opacity, end2.opacity);
    return function(t4) {
      start3.h = h2(t4);
      start3.s = s(t4);
      start3.l = l(t4);
...
function hcl4(hue3) {
  return function(start3, end2) {
    var h2 = hue3((start3 = hcl3(start3)).h, (end2 = hcl3(end2)).h), c3 = nogamma2(start3.c, end2.c), l = nogamma2(start3.l, end2.l), opacity = nogamma2(start3.opacity, end2.opacity);
    return function(t4) {
      start3.h = h2(t4);
      start3.c = c3(t4);
      start3.l = l(t4);
...
function cubehelix4(hue3) {
  return function cubehelixGamma(y) {
    y = +y;
    function cubehelix5(start3, end2) {
      var h2 = hue3((start3 = cubehelix3(start3)).h, (end2 = cubehelix3(end2)).h), s = nogamma2(start3.s, end2.s), l = nogamma2(start3.l, end2.l), opacity = nogamma2(start3.opacity, end2.opacity);
      return function(t4) {
        start3.h = h2(t4);
        start3.s = s(t4);
        start3.l = l(Math.pow(t4, y));
...
function transformer() {
  var domain = unit, range = unit, interpolate = value_default2, transform, untransform, unknown, clamp = identity3, piecewise3, output, input;
  function rescale() {
    var n2 = Math.min(domain.length, range.length);
    if (clamp !== identity3)
      clamp = clamper(domain[0], domain[n2 - 1]);
    piecewise3 = n2 > 2 ? polymap : bimap;
    output = input = null;
    return scale;
  }
  function scale(x) {
    return x == null || isNaN(x = +x) ? unknown : (output || (output = piecewise3(domain.map(transform), range, interpolate)))(transform(clamp(x)));
  }
  scale.invert = function(y) {
    return clamp(untransform((input || (input = piecewise3(range, domain.map(transform), number_default3)))(y)));
  };
  scale.domain = function(_) {
...
var F = { archimedean: E3, rectangular: function(t4) {
...
function U(t4) {
  ...
  (n2 = x ? function() {
    ...
    return d2.canvas = function(t6) {
      return arguments.length ? (m2 = D3(t6), d2) : m2;
    }, d2.start = function() {
      var f3 = function(t6) {
        ...
        return function n4(r4) {
          ...
          !function(n5, r5) {
            for (var e7 = n5; e7 < r5; e7 += 1) {
              var o5 = g[e7];
              o5.x = t5[0] * (h2() + 0.5) >> 1, o5.y = t5[1] * (h2() + 0.5) >> 1, A3(f3, o5, g, e7), o5.hasText && x2(s2, o5, p) && (v.push(o5), c3.call("word", d2, o5), p ? q(p, o5) : p = [{ x: o5.x + o5.x0, y: o5.y + o5.y0 }, { x: o5.x + o5.x1, y: o5.y + o5.y1 }], o5.x -= t5[0] >> 1, o5.y -= t5[1] >> 1);
            }
      ...
      return arguments.length ? (r3 = D3(t6), d2) : r3;
    }, d2.fontStyle = function(t6) {
      return arguments.length ? (o3 = D3(t6), d2) : o3;
    }, d2.fontWeight = function(t6) {
      return arguments.length ? (a2 = D3(t6), d2) : a2;
    }, d2.rotate = function(t6) {
      return arguments.length ? (i = D3(t6), d2) : i;
    }, d2.text = function(t6) {
      return arguments.length ? (n3 = D3(t6), d2) : n3;
    }, d2.spiral = function(t6) {
      return arguments.length ? (f2 = F[t6] || t6, d2) : f2;
    }, d2.fontSize = function(t6) {
      return arguments.length ? (e4 = D3(t6), d2) : e4;
    }, d2.padding = function(t6) {
      return arguments.length ? (u2 = D3(t6), d2) : u2;
  ...
  }() : (0, import_d3_cloud.default)()).size(u).padding(B4).words((0, import_lodash2.default)(X)).rotate(function() {
...
var B3 = { colors: range_default(20).map(function(t4) {
  return t4.toString();
...
function G(i) {
  var f = i.callbacks, l = i.maxWords, s = void 0 === l ? 100 : l, c3 = i.minSize, h2 = i.options, d2 = i.size, m2 = i.words, y = function(t4, n2) {
    ...
    return o2;
  }(i, ["callbacks", "maxWords", "minSize", "options", "size", "words"]), p = M({}, H, f), x = M({}, B3, h2), v = function(t4, n2, i2) {
...
G.defaultProps = { callbacks: H, maxWords: 100, minSize: [300, 300], options: B3 };
var index_module_default = G;
export {
  index_module_default as default,
  H as defaultCallbacks,
  B3 as defaultOptions
};
